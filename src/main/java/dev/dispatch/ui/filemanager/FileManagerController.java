...
/**
 * Root controller for the Midnight Commander-style dual-panel file manager. Manages two {@link
 * FilePanelController} instances and handles all file operations.
 */
public class FileManagerController {
  ...
  /**
   * Loads the FXML, initialises both panels, wires keyboard shortcuts. Must be called from the FX
   * Application Thread.
   */
  public Node createNode() {
    try {
      FXMLLoader loader =
          new FXMLLoader(getClass().getResource("/dev/dispatch/fxml/file-manager.fxml"));
                  ...
                  .start(
                      () -> {
                        try {
                          ...
                          Platform.runLater(
                              () -> showError("Create directory failed", e.getMessage()));
                        }
                      });
            });
  }
  ...
  private void onDelete() {
    ...
    String preview =
        sel.stream().map(FileEntry::getName).limit(4).reduce("", (a, b) -> a + "\n  " + b);
    if (sel.size() > 4) preview += "\n  … and " + (sel.size() - 4) + " more";
    Alert confirm =
        new Alert(
            Alert.AlertType.CONFIRMATION,
            "Delete " + sel.size() + " item(s)?" + preview,
            ButtonType.OK,
            ButtonType.CANCEL);
              ...
              for (FileEntry entry : sel) {
                ...
                if (error[0] != null) {
                  ...
                  Platform.runLater(
                      () -> {
                        dialog.hide();
                        showError("Transfer failed", err.getMessage());
                      });
                  return;
                }
  ...
  /**
   * Called from VT after each DnD item completes. Returns the dialog's running total so the caller
   * can use it as {@code transferredBefore} for the next item (correct for directories whose {@link
   * FileEntry#getSize()} is 0).
   */
  ...
  private TransferProgressDialog getDialog() {
    if (progressDialog == null) {
      Window owner =
          rootNode != null && rootNode.getScene() != null ? rootNode.getScene().getWindow() : null;
  ...
  private void deleteQuietly(FileSession sess, FileEntry entry) {
    try {
      sess.delete(entry.getPath(), true);
    } catch (SftpException e) {
      log.error("delete failed: {}", entry.getPath(), e);
    }
  }
