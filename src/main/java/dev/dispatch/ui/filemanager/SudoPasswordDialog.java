...
/**
 * Modal dialog that prompts the user for a sudo password when the file manager encounters a
 * permission-denied error while navigating a remote directory.
 *
 * <p>Returns the entered password, or {@code null} if the user cancelled. Must be shown on the FX
 * Application Thread.
 */
public class SudoPasswordDialog extends Dialog<String> {
  ...
  public SudoPasswordDialog(String deniedPath, Window owner) {
    ...
    Label info =
        new Label(
            "Permission denied for directory:\n"
                + deniedPath
                + "\n\nEnter your sudo password to continue as root.");
    ...
    getDialogPane()
        .getStylesheets()
        .add(getClass().getResource("/css/dispatch-dark.css").toExternalForm());
