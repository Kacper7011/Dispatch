...
/**
 * {@link FileSession} implementation that runs all operations via {@code sudo -S} over an existing
 * SSH connection. Used when the regular SFTP session is denied access to a path.
 *
 * <p>The {@code sudo} password is supplied once at construction and injected into each command's
 * stdin. All blocking operations must be called from a virtual thread.
 *
 * <p>This session does not own the underlying {@link SshSession} and therefore does not close it in
 * {@link #close()}.
 */
public final class SudoSshFileSession implements FileSession {
  ...
  public void upload(InputStream in, String destPath, long size, TransferMonitor monitor) {
    ...
    InputStream stdinStream = new SequenceInputStream(new ByteArrayInputStream(passwordBytes), in);
  ...
  public boolean isDirectory(String path) {
    dev.dispatch.ssh.ExecResult r =
        ssh.execWithStdin(SUDO + "test -d " + shellEscape(path), passwordBytes);
  ...
  public String realpath(String path) {
    dev.dispatch.ssh.ExecResult r =
        ssh.execWithStdin(SUDO + "realpath " + shellEscape(path), passwordBytes);
  ...
  private List<FileEntry> parseLsOutput(String dir, String stdout) {
    ...
    for (String line : stdout.split("\n")) {
      ...
      try {
        modified = LocalDateTime.parse(parts[5]);
      } catch (Exception e) {
      ...
      String name =
          (isSymlink && rest.contains(" -> ")) ? rest.substring(0, rest.indexOf(" -> ")) : rest;
  ...
  /**
   * Wraps {@code s} in single quotes and escapes embedded single quotes so the result is safe to
   * embed in a shell command string (e.g. paths with spaces or apostrophes).
   */
